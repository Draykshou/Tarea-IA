public class GrafosRecorridos {
	...
	public void Dijkstra(Lista<Vertice> G, int Origen){
		// Voy a actualizar la distancia del nodo inicial como 0
		if(!G.Buscar(new Vertice(Origen))){
			System.out.println("EL GRAFO NO CONTIENE EL NODO ORIGEN");
			return;
		}
		...
		while (Q.Retirar()){
			...
			if(!G.Buscar(new Vertice(U))){
				System.out.println("ERROR EN LA ESTRUTUTA DEL GRAFO "+U);
				return;
			}		
